   ...
   SUBROUTINE trc_bc(kt, jit)
      ...
      IF ( PRESENT(jit) ) THEN 
         ...
         IF( nb_trcobc > 0 ) THEN
           ...
           DO ib = 1,nb_bdy
             CALL fld_read(kt=kt, kn_fsbc=1, sd=sf_trcobc(:,ib), map=nbmap_ptr(:,ib), kit=jit, kt_offset=+1, jpk_bdy=nb_trc_jpk_bdy(ib), fvl=.FALSE.)
           ENDDO
         ENDIF
      ...
      ELSE
         ...
         IF( nb_trcobc > 0 ) THEN
           if (lwp) write(numout,'(a,i5,a,i10)') '   reading OBC data for ', nb_trcobc ,' variable(s) at step ', kt
           DO ib = 1,nb_bdy
             CALL fld_read(kt=kt, kn_fsbc=1, sd=sf_trcobc(:,ib), map=nbmap_ptr(:,ib), kt_offset=+1, jpk_bdy=nb_trc_jpk_bdy(ib), fvl=.FALSE.)
           ENDDO
         ENDIF
